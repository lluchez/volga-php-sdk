...
--
-- Host: localhost
-- Generation Time: Oct 09, 2010 at 10:27 AM
...
--
-- Stand-in structure for view `loc_view`
--
DROP VIEW IF EXISTS `loc_view`;
CREATE TABLE IF NOT EXISTS `loc_view` (
`locID` smallint(6)
,`key` varchar(50)
,`type` enum('html','text','js','int','float')
,`tiny` tinyint(1)
,`admin` tinyint(1)
,`locIdx` smallint(6)
,`langIdx` char(3)
...
--
-- Structure for view `loc_view`
--
DROP TABLE IF EXISTS `loc_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `loc_view` AS select `k`.`locID` AS `locID`,`k`.`key` AS `key`,`k`.`type` AS `type`,`k`.`tiny` AS `tiny`,`k`.`admin` AS `admin`,`t`.`locIdx` AS `locIdx`,`t`.`langIdx` AS `langIdx`,`t`.`content` AS `content` from (`loc_text` `t` join `loc_key` `k` on((`k`.`locID` = `t`.`locIdx`)));
...
ALTER TABLE `loc_text`
  ADD CONSTRAINT `loc_text_ibfk_1` FOREIGN KEY (`locIdx`) REFERENCES `loc_key` (`locID`),
  ADD CONSTRAINT `loc_text_ibfk_2` FOREIGN KEY (`langIdx`) REFERENCES `language` (`langID`);
